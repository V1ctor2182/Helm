...
- [x] [F1][P1](本轮清) 链接分类三层(2026-07-08 用户拍板,详见 docs/AI-SYSTEM.md):enrich 的 type 拆成 family(video|paper|design|link 视觉族,死枚举)+ label(自由但归一化的规范类别,注入已有 label 优先复用+alias 兜底);topic 不变。type 保留兼容。前端墙卡/详情显示 label chip + family badge;侧栏筛选兼容 family。契约只加不减,通知 notch。
