...
- [ ] K5 日记页详情:全页阅读态+当日碎片时间线(该日 notes)+专注块
- [ ] K6 任务操作台:filter=task 视图改 双列(待办 checkbox 清单+定时卡:渐变开关/cron chip/运行历史抽屉);派发条胶囊化
- [ ] K7 智能判类:后端轻量 classify(enrich 前置或独立)+捕获条(CaptureDock/smartcap)实时判定徽章(先规则,AI 兜底)+可点改判;发送按判定落库
