# （已归档 v1）F5 · Agent 编排与工具（融合粘合层）

> ⚠️ **已归档 — v1，归档于 2026-06-16**
>
> 本版本设计的是「**双 agent**」方案：外部 Claude Code/Codex + Helm **内置/本地 agent**（Odysseus 路线，Python 后端带工具）。
>
> **现行决策**：暂不做内置/本地 agent，**统一默认用 Claude / Claude Code**（仅保留外部 CLI 路线）。
>
> 当前生效版本见 → [../features/F5-agent-orchestration.md](../features/F5-agent-orchestration.md)。
>
> 本文件仅作历史保留：若未来恢复"内置 agent"方案，可从此处取回设计。下文为归档前的原始内容。
