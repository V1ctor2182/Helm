...
## ⏸️ 已暂停 · #27 — 真实 IMAP 收邮件

> **2026-06-28：邮件能力已暂停**（用户决定之后再加；UI 入口 ✉ Mail 与后端 `/api/mail` 路由已下线，`helm/mail/` 代码保留）。
> **这条现在不用做了。** issue #27 已关（标注暂停）。等邮件加回来再跑下面的步骤。

<details>
<summary>（折叠保留：邮件加回后照这个验）</summary>
...
## 🟡 第 1 条（小额花钱，重要）· #19 — 真实 Claude Code 核对格式
...
| 顺序 | ticket | 谁做 | 花钱 | 一句话 |
|---|---|---|---|---|
| 1 | #19 真 Claude Code | 你跑 | 小额 | 跑 stream-json 贴我 |
| 2 | #23 真 Deep Research | 你跑 | 是 | 配 provider→跑研究 |
| 3 | #25 后台调度 | 你跑 | 是 | HELM_SCHEDULER=1→建任务 |
| 4 | #12 httpx2 | 你拍板 | — | 回我 wontfix / 迁 |
| 5 | #8/#10/#13 打包簇 | 你的环境 | — | 出安装包时再说 |
| 6 | #16 Skills 集成 | 待设计 | — | 最后做 |
| ⏸️ | ~~#27 真 IMAP~~ | — | — | **邮件已暂停,之后再加** |

> 已完成(今天)：#22 引擎方向(决策)、#28 CalDAV(实现)、中文搜索 bug(白赚修复)、停用邮件+日历移入 Journal。
