...
# 阶段 3.5 · 驾驶舱 × FanBox 行为对齐(结构层:交互骨架)

> **触发时机:** 阶段 3 微观行为 P1 全勾后,实测手感仍差——差距在**宏观交互骨架**(2026-07-03 与 FanBox 真机并排对比结论,见 F1 room)。本阶段把骨架对齐:布局/导航/面板机制照搬 FanBox,皮肤仍守 DESIGN.md。
>
> 立场不变:**FanBox 是行为参考不是视觉参考**;冲突时 DESIGN.md 赢。参考仓/账本/每轮 10 步/硬规则/附录 全部沿用阶段 3 原文,唯一不同是选条范围=矩阵「P0·结构」层。

## 矩阵新增「P0·结构」层(本阶段的账本,首轮建立)

按伤害排序,五件套:

1. **预览按需面板**(最大元凶):无选中时文件区铺满全宽;选中才滑出预览;Esc/✕ 关闭回全宽;中缝可拖(比例存 localStorage)。FanBox:预览是 on-demand 面板,文件区永远是主角。
2. **左侧栏**:搜索入口(⌘K)、快速入口(Home/桌面/文稿/下载)、收藏(星标可增删)、**Agent 项目带活跃度徽章**(just now/2m,来源=变更事件时间);可折叠。
3. **可点面包屑**:cwd 逐段可点跳转,替代死路径字符串;保留 ↑。
4. **终端收进驾驶舱做底部 dock**:文件区下方,可拖高、可铺满、可折叠;替代 Shell 级 40px 边条(多标签可后置 P2)。
5. **网格呼吸感**:图标/间距/字号上调一档,向 FanBox 的从容尺度靠(密度 token 仍走 DESIGN.md)。

每轮做一件(大件可拆两轮);完成条件:**P0·结构 5 件全勾(或 wontfix 注明)→ 报告并停**。缺陷复查仍记 backlog,P0/P1 缺陷优先。
