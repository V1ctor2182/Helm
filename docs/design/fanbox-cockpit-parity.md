...
- [ ] **终端路径点击(四层识别)+ 点开定位** | URL>引号串>含斜杠 token(服务端 stat 批量验证)>裸文件名扩展白名单;目录尾 `/` 兜底;折行按 cell 拼回;点击→cwd 解析→回扫 scrollback→basename 搜索兜底;md/html 点开全屏 | app:2528-2644, srv:937-1018
- [ ] **终端状态感知:busy/idle/dead 圆点 + 完成提醒** | 输出→busy(脉冲)、静默 2.5s→idle、进程退→dead;回显过滤(<400ms 不算 agent 干活);busy>4s 收工→涟漪+双音+通知;「esc to interrupt」30s 护栏;「等你拍板」检测单音 | app:2692-2801
...
|---|---|---|
| 1 | 建矩阵(本文件) | — |
| 2 | 预览即编辑+自动落盘+并发保护 | (见 git log 轮2) |
