...
- [ ] **预览即编辑 + 停笔 0.8s 自动落盘 + 并发覆盖保护** | helm 预览完全只读,intent 的「轻改」整个缺失。代码/纯文本进预览即可编辑;防抖自动保存(写盘串行化+⌘S 立即);写盘带 expectedMtime,被 agent 外改弹覆盖确认;后端原子写(tmp+fsync+rename);>2MB 拒编辑;「xx 之前已保存」字样 | app:1179-1382, srv:409-433
