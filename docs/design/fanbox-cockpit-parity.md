...
## P0·结构(阶段 3.5,2026-07-03 真机并排对比后补层——微观行为都在,骨架不对手感就不对)

- [x] **预览按需面板** → 3.5轮1 搬入(无选中铺满全宽——真机网格 3 列→7 列;选中滑出+可拖中缝 28-72% 存 localStorage;×/Esc 关回全宽;Agent 观察台 rightTab 不依赖选中,viewbar AGENT 钮直开) | 对比结论#1
- [x] **左侧栏** → 3.5轮2 搬入(搜索钮开 ⌘K/快速入口 Home·桌面·文稿·下载/收藏星标(右键目录收藏+侧栏×取消,localStorage)/Agent 项目+watch 驱动活跃度徽章 刚刚·Nm·Nh·Nd(持久)/≡ 折叠开关/当前目录 accent 左沿高亮) | fb 侧栏
- [ ] **可点面包屑**:cwd 逐段可点;保留 ↑ | fb 顶栏
- [x] **终端底部 dock → 升级为全模块 docking**(用户拍板超纲 FanBox):文件/预览/终端/Agent 全部 dock 模块,拖 tab 换 zone 吸附高亮/同 zone tab 栈/右缝底缝拖尺寸/zone 折叠条/布局 localStorage;模块 DOM 迁移零重挂(xterm 缓冲实测保留);Shell 终端边条退役 | fb 终端面板+超纲
- [ ] **网格呼吸感**:图标/间距/字号上调一档 | fb 主区尺度
...
- [x] **拖文件进终端** → 轮13 搬入(卡片/列表行可拖:text/plain+x-helm-path+图片 text/html 原路径;终端 dragover accent 框+「松手喂给 agent」提示;drop 插入 shQuote 转义路径+空格,视同用户输入;window 级 dragover/drop 兜底防导航;Finder 外部文件拖入落盘留 P2) | 卡片可拖,落进终端插入 shell 单引号转义路径;dragover 虚线高亮;全局 drop 兜底防导航 | app:465-511, 2060-2090
...
- [x] **跟随模式升级:归属双判定 + 节流优先级 + 代码变动行高亮** → 轮14 搬入(agent 活跃门 busy/8s;节流非防抖:首切 120ms 后 900ms、定时器只设一次取最新、低优先级不顶 md/html;同文件 followTick 只刷视图;编辑器 dirty 不抢屏;手动接管即停(点文件/openPath/编辑);开启即回溯 5min;代码重载滚到首变动行+绿脉冲、md 尾变贴底中间保视口;html 双缓冲随 HTML 交互预览 P2) | 只跟绑定终端 cwd 且该 agent 正忙;多文件快写节流切目标(html/md>代码>其它);代码重读全文夹逼算变动行区间闪高亮平滑滚;md 尾部贴底/中间不跳;html 双缓冲零白闪 | app:3360-3661
...
- [x] HTML 交互预览(从 P2 提级,用户拍板)→ 隔离预览源 127.0.0.1:8770(独立 origin=沙箱本体,stdlib 线程服务,限 $HOME/拒穿越)+ 预览/源码/Diff tab + ↻/⧉ + 双缓冲 iframe 零白闪 + watch 自动刷新(实测 v1→v2)(sandbox 隔离端口+定宽自动缩放+本地图兜底)——工程大,先 P2 | srv:2120-2143
...
- [ ] skill 行拖进终端(/name·$name 按 agent 注入) | app:3054-3086
...
| 12 | 变更收件箱+噪声过滤(并轮) | a46c202 |
| 13 | 拖文件进终端 | 9808e7f |
| 14 | 跟随模式升级(末条 P1) | b9a510a |
| 3.5-1 | 预览按需面板(结构#1) | 3ad20bc |
| 3.5-2 | 左侧栏(结构#2) | (见 git log) |
