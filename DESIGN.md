...
- **设计稿**：
  ...
  - `docs/design/helm-notch-nomi.html` — **⭐ SOURCE OF TRUTH（notch 下一代定稿，2026-07-06 用户拍板：logo + NOMI 风）**：黑刘海（白 logo + 橙紫渐变波形 + 时钟）点击垂下白色大圆角面板（26px 圆角 + 深阴影）；四模块黑胶囊 tab——捕获（5 kind 胶囊 + 任务给自己/交给 agent 渐变切换 + 灰盒输入 + 渐变发送 + 问大脑答案卡）、媒体（封面 + 渐变进度条 + 黑圆播放钮 + 歌词行）、会话（live/等回答/idle 状态点卡 + vibeisland 选择题渐变按钮）、日历（渐变时间条事件行）；底部快捷键行 + 「打开 Helm」。定稿后按 notch loop 老规矩逐块进 Swift；现行 Swift 实现的真相源在切换前仍是 `notch/docs/helm-notch-pro.html`。
  - `docs/design/helm-pro.html` — 旧 ORAGE 座舱定稿（外壳+Today，2026-07-06 被 NOMI 全站稿取代，存档）。
...
## 层级三层制 + 左内右外(2026-07-05 用户定稿,参照稿 `docs/design/explore-hier-ac.html` v3)

> 病根:全屏字号挤在 9-13px 窄带、明暗对比不足 → 层级平、没有重点。
> 三方向探索(A 字号/B 重量/C 亮度)后用户拍板 A×C 合成,三轮迭代收敛为:

1. **屏锚层(恒亮)**:每屏一个超大主读数——Today 72px 重磅 mono 时钟(+22px 日期),驾驶舱 28px 项目名。永远 t1 全亮,不参与聚光,是屏幕的固定锚。
2. **区块聚光制**:内容区块构成聚光系统——焦点岛=背景提亮一档(dark `#0d0d10`/light `#f2efe6`)+左缘 2px accent 实条+文字 t1/t2;非焦点=透明背景+t3/t4;点击任意区块 280ms `cubic-bezier(.32,.72,0,1)` 气动移光。**一屏有且只有一个亮岛**。
3. **数字回区块头(大而暗)**:每区块头行内右端挂该区块的 22px mono key 数字(发丝线撑开);焦点区 accent-ink,非焦点区**保持 22px 压 t4**——眯眼仍读得到,层级全靠亮度。
4. **左内右外(信息架构)**:左列=我的一天(任务/日记/AGENT/日程,聚光在此);右柱=外面的世界(~300px 常亮简报柱:新闻/邮件分诊/未来信息源,不参与聚光)。驾驶舱右柱=预览+RUN,不放新闻。
5. **字阶五级**:9(mono 标签)/12(辅文)/15(正文)/22(key 数字)/72&28(屏锚)。中间不加档。

token/双主题/禁 emoji/零圆角/点阵底纹仍承 `helm-pro.html`;本章只改层级与信息动线。
